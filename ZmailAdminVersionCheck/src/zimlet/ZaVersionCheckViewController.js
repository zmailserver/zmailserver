...
function () {
	...
	this._popupOperations[ZaOperation.VERSION_CHECK] = new ZaOperation(ZaOperation.VERSION_CHECK, org_zmail_adminversioncheck.CheckNow, org_zmail_adminversioncheck.CheckNow_tt, "Refresh", "Refresh", new AjxListener(this, this.checkNowListener));
}
...
function(ev) {
	if(this._view.isDirty()) {
		this.popupMsgDialog(org_zmail_adminversioncheck.saveChangesFirst, true);		
	} else if (AjxUtil.isEmpty(this._currentObject.attrs[ZaVersionCheck.A_zmailVersionCheckServer]) || 
				AjxUtil.isEmpty(this._currentObject.attrs[ZaVersionCheck.A_zmailVersionCheckURL]) ||
				(this._currentObject.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationEmail] == "TRUE" &&
					(
			 		 AjxUtil.isEmpty(this._currentObject.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationSubject]) ||
			 		 AjxUtil.isEmpty(this._currentObject.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationBody]) ||
			 		 AjxUtil.isEmpty(this._currentObject.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationEmail])
			 		)
			    )
			) {
		this.popupMsgDialog(org_zmail_adminversioncheck.ERROR_UPDATES_NOT_CONFIGURED, true);
	} else {
...
function () {
	...
	//check values
	if(!AjxUtil.isEmpty(tmpObj.attrs[ZaVersionCheck.A_zmailVersionCheckSendNotifications]) && tmpObj.attrs[ZaVersionCheck.A_zmailVersionCheckSendNotifications]=="TRUE") {
		if(AjxUtil.isEmpty(tmpObj.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationEmail])) {
			//show error msg
			this._errorDialog.setMessage(AjxMessageFormat.format(ZaMsg.ERROR_INVALID_VALUE_FOR, [org_zmail_adminversioncheck.MSG_zmailVersionCheckNotificationEmail]), null, DwtMessageDialog.CRITICAL_STYLE, null);
			this._errorDialog.popup();		
			return false;
		}
				
		if(AjxUtil.isEmpty(tmpObj.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationBody])) {
			//show error msg
			this._errorDialog.setMessage(AjxMessageFormat.format(ZaMsg.ERROR_INVALID_VALUE_FOR, [org_zmail_adminversioncheck.MSG_zmailVersionCheckNotificationBody]), null, DwtMessageDialog.CRITICAL_STYLE, null);
			this._errorDialog.popup();		
			return false;
		}
		
		if(AjxUtil.isEmpty(tmpObj.attrs[ZaVersionCheck.A_zmailVersionCheckNotificationSubject])) {
			//show error msg
			this._errorDialog.setMessage(AjxMessageFormat.format(ZaMsg.ERROR_INVALID_VALUE_FOR, [org_zmail_adminversioncheck.MSG_zmailVersionCheckNotificationSubject]), null, DwtMessageDialog.CRITICAL_STYLE, null);
			this._errorDialog.popup();		
			return false;
		}
	}
