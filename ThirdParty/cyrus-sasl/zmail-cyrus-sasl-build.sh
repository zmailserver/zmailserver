...
#
# Builds a saslauthd with Zmail authentication support.  We modify
...
build_platform=`sh ${p4_root}/ZmailBuild/rpmconf/Build/get_plat_tag.sh`
...
LIBS="/opt/zmail/libxml2/lib/libxml2.a" CFLAGS="-D_REENTRANT -g -O2 -I/opt/zmail/libxml2/include/libxml2" ./configure --enable-zmail --prefix=/opt/zmail/${cyrus_src} \
            --with-saslauthd=/opt/zmail/data/sasl2/state \
            --with-plugindir=/opt/zmail/${cyrus_src}/lib/sasl2 \
            --enable-static=no \
            --enable-shared \
            --with-dblib=no \
            --with-devrandom=/dev/urandom \
            --with-openssl=/opt/zmail/openssl-${openssl_version} \
            --with-libcurl=/opt/zmail/curl-${curl_version} \
            --with-gss_impl=heimdal \
            --enable-gssapi=/opt/zmail/heimdal-${heimdal_version} \
            --with-libxml2=/opt/zmail/libxml2-${xml2_version}/bin/xml2-config \
            --with-configdir=/opt/zmail/conf/sasl2 \
            --enable-login
else
	if [ $build_platform = "UBUNTU12_64" ]; then
		xmllib='/opt/zmail/libxml2/lib/libxml2.a -lz -lm'
	else
		xmllib='/opt/zmail/libxml2/lib/libxml2.a'
	fi
LIBS="${xmllib}" CFLAGS="-D_REENTRANT -g -O2 -I/opt/zmail/libxml2/include/libxml2" ./configure --enable-zmail --prefix=/opt/zmail/${cyrus_src} \
            --with-saslauthd=/opt/zmail/data/sasl2/state \
            --with-plugindir=/opt/zmail/${cyrus_src}/lib/sasl2 \
            --with-dblib=no \
            --with-devrandom=/dev/urandom \
...
if [ $platform = "Darwin" ]; then
     ...
     sed -i .bak -e "s|-L/opt/zmail/libxml2-${xml2_version}/lib -lxml2||" saslauthd/Makefile
