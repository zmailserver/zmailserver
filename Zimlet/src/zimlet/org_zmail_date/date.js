...
Com_Zmail_Date = function(pattern) {
...
Com_Zmail_Date.prototype = new ZmZimletBase();
Com_Zmail_Date.prototype.constructor = Com_Zmail_Date;

//
// Data
//

Com_Zmail_Date.prototype.TYPE = ZmObjectManager.DATE;

//
// Public methods
//

Com_Zmail_Date.validate =
...
Com_Zmail_Date.prototype.getCurrentDate =
function(date) {
...
Com_Zmail_Date.prototype.init =
function() {
	Com_Zmail_Date.prototype._zimletContext = this._zimletContext;
	Com_Zmail_Date.prototype._className = "Object";
	this._initDateObjectHandlers();
};

Com_Zmail_Date.prototype.getActionMenu =
function(obj, span, context) {
	...
	Com_Zmail_Date._actionObject = obj;
	Com_Zmail_Date._actionSpan = span;
	Com_Zmail_Date._actionContext = context;
	return this._zimletContext._contentActionMenu;
};

Com_Zmail_Date.prototype.menuItemSelected =
function(itemId) {
	switch (itemId) {
		case "DAYVIEW":		this.showDayView(Com_Zmail_Date._actionContext.date); break;
		case "NEWAPPT":		this._newApptListener(); break;
		case "SEARCHMAIL":	this._searchMailListener(); break;
	}
};

Com_Zmail_Date.prototype.toolTipPoppedUp =
 ...
 * @param appId
 */
Com_Zmail_Date.prototype.getMainWindow =
function(appId) {
 ...
 * @param appId
 */
Com_Zmail_Date.prototype.getAppInMainWindow =
function(appId) {
...
Com_Zmail_Date.prototype.clicked =
function(spanElement, contentObjText, matchContext, canvas) {
	this.showDayView(matchContext.date);
};

Com_Zmail_Date.prototype.showDayView =
function(date) {
	var app = this.getAppInMainWindow(ZmApp.CALENDAR);
	if (!app) {
		return;
...
Com_Zmail_Date.prototype.match =
function(line, startIndex) {
	// is there anything to do?
	if (!Com_Zmail_Date.PATTERNS) { return null; }

	// find first match
	var match, mapping, rule, re, m, i;
	for (i = 0; i < Com_Zmail_Date.REGEXES.length; i++) {
		re = Com_Zmail_Date.REGEXES[i];
		re.lastIndex = startIndex;
		m = re.exec(line);
		if (m && m[0] && (!match || m[0].length > match[0].length )) { // Longest match wins
			match = m;
			rule = Com_Zmail_Date.RULES[i];
			mapping = re.mapping;
			break;
		}
	}
...
function() {
	...
	Com_Zmail_Date.PATTERNS = [];
	Com_Zmail_Date.RULES = [];
	Com_Zmail_Date.REGEXES = [];
	...
	for (i = 1; pattern = this.getMessage("format"+i+".pattern"); i++) {
		...
		Com_Zmail_Date.PATTERNS.push(pattern);
		Com_Zmail_Date.RULES.push(this.getMessage("format"+i+".rule"));
	}
	for (i = 0; i < Com_Zmail_Date.DEFAULT_FORMATS.length; i++) {
		Com_Zmail_Date.PATTERNS.push(Com_Zmail_Date.DEFAULT_FORMATS[i]);
		Com_Zmail_Date.RULES.push(Com_Zmail_Date.DEFAULT_FORMATS[++i]);
	}
	...
	for (i = 0; i < Com_Zmail_Date.PATTERNS.length; i++) {
		pattern = Com_Zmail_Date.PATTERNS[i];
...
//        pattern = pattern.replace(/\s+/g, "\\b\\s*\\b");
		...
		Com_Zmail_Date.__replaceKeyword_group = 1;
		Com_Zmail_Date.__replaceKeyword_mapping = {};
		pattern = pattern.replace(/\{([a-z]+)\}/g, Com_Zmail_Date.__replaceKeyword);
		...
		regex.mapping = Com_Zmail_Date.__replaceKeyword_mapping;
		Com_Zmail_Date.REGEXES.push(regex);
...
Com_Zmail_Date.prototype._newApptListener =
function() {
	var win = this.getMainWindow();
	if (!win) {
		return;
...
Com_Zmail_Date.prototype._handleLoadNewAppt =
function() {
	...
	win.AjxDispatcher.run("GetCalController").newAppointmentHelper(Com_Zmail_Date._actionContext.date);
	if (appCtxt.isChildWindow) {
		window.opener.focus();
	}
};

Com_Zmail_Date.prototype._searchMailListener =
function() {
	...
	win.appCtxt.getSearchController().dateSearch(Com_Zmail_Date._actionContext.date);
	if (appCtxt.isChildWindow) {
		window.opener.focus();
	}
};
...
Com_Zmail_Date.__replaceKeyword_mapping = null;
Com_Zmail_Date.__replaceKeyword_group = -1;

Com_Zmail_Date.__replaceKeyword =
function($0, keyword) {
	var MAPPINGS = Com_Zmail_Date.MAPPINGS;
	...
	var mapping = Com_Zmail_Date.__replaceKeyword_mapping;
	var group = Com_Zmail_Date.__replaceKeyword_group++;
