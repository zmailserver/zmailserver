...
ECHO symstore add /f %BASE%\src\c\*\rtl\*.* /s \\%INDEX_HOST%\Zbuild3\symbols\ZCO /t "Zmail Migration Tools" /v "%BUILD_VERSION%" /c "%BUILD_DATE%"
symstore add /f %BASE%\src\c\Win32\rtl\*.* /s \\%INDEX_HOST%\Zbuild3\symbols\ZCO /t "Zmail Migration Tools x86" /v "%BUILD_VERSION%" /c "%BUILD_DATE%"
IF ERRORLEVEL 1 exit /B 1 
symstore add /f %BASE%\src\c\x64\rtl\*.* /s \\%INDEX_HOST%\Zbuild3\symbols\ZCO /t "Zmail Migration Tools x64" /v "%BUILD_VERSION%" /c "%BUILD_DATE%"
IF ERRORLEVEL 1 exit /B 1 
