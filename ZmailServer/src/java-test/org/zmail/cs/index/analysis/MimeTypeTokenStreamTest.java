...
public final class MimeTypeTokenStreamTest {
    ...
    public void tokenize() throws Exception {
        ...
        Assert.assertEquals(Arrays.asList("image/jpeg", "image", "text/plain", "text", "text/foo/bar", "text",
                "aaa bbb ccc ddd/eee fff/ggg/hhh", "aaa bbb ccc ddd", "any"), ZmailAnalyzerTest.toTokens(stream));
    }

    @Test
    public void limit() throws Exception {
        MimeTypeTokenStream stream = new MimeTypeTokenStream("x");
        Assert.assertEquals(Arrays.asList("none"), ZmailAnalyzerTest.toTokens(stream));
        ...
        Assert.assertEquals(101, ZmailAnalyzerTest.toTokens(stream).size());
    }
