...
class MailboxErrorUtil {
    ...
    static void handleCascadeFailure(Mailbox mbox, List<Integer> cascadeIds, ServiceException e) throws ServiceException {
        if (causeMatchesFKFailure(e)) {
            ZmailLog.mailbox.error("deleting cascadeIds failed due to foreign key constraint failed; attempting to delete individually and find failure");
            ...
            for (Integer id: cascadeIds) {
              try {
                  ...
                  ZmailLog.mailbox.debug("attempting to delete id ["+id+"]");
                  DbMailItem.delete(mbox, singleItemList, false);
                  ZmailLog.mailbox.debug("deleted ["+id+"] OK");
              } catch (ServiceException se) {
                  ZmailLog.mailbox.error("deleted FAILED for ["+id+"] due to exception",se);
                  failures.add(id);
              }
            }
            if (!failures.isEmpty()) {
                ...
                for (Integer id: failures) {
                    ...
                    ZmailLog.mailbox.error(logMsg);
                    if (item instanceof Conversation) {
                        Conversation conv = (Conversation) item;
                        List<Message> children = conv.getMessages();
                        if (children != null && children.size() > 0) {
                            ZmailLog.mailbox.error("converstaion["+conv.getId()+"] still has "+children.size()+" children.");
                        ...
                        if (children != null && children.size() > 0) {
                            ZmailLog.mailbox.error("folder["+folder.getId()+"] still has "+children.size()+" children.");
                        ...
                        ZmailLog.mailbox.warn("cascade failure in unexpected type ["+item.getType()+":"+item.getClass().getSimpleName()+"] other than folder or conversation");
                    }
                }
